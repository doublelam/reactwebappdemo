...
export class AllotList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      verifyStatus: [],
              ...
              { title: '审核状态', dataIndex: 'status', width: 100 },
              {
                title: '操作', width: 100, render: (row) => {
                  if (row.status === '完成') {
                    return (
                      <div>
                        <Link to={'allotquitnetdetail' + '?pendingId=' + row.pendingId + '&editable=false'}>详情</Link>
                        <span> </span>
                        <Link to={'allotquitnetdetail' + '?pendingId=' + row.pendingId}>退网</Link>
                      </div>
                    )
                  }
                  return <Link to={'allotquitnetdetail' + '?pendingId=' + row.pendingId}>详情</Link>
                },
              },
            ]}
          />
  ...
  getVerifyStatus() {
    post('allot/logout/getLogoutStatus').then(data => {
      if (!data.success) {
