...
export class MediaReceiptCount extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
      ...
      <section className={publicStyles.section}>
        <QueryHeader style={{
          ...
          <div className={publicStyles.paddingWith} style={{
            ...
            >
              {this.state.cashType.map((val, index) => {
                return (
                  <Select.Option
                    key={val}
                  >{val}</Select.Option>
          ...
          <div className={publicStyles.paddingWith}
            hidden={
  ...
  getStateInfo(ifInit, ifPageChange) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      // }
      let conditionFilter = [];
      if (this.state.portType === 'netdots' && ObjOperate.trimExtra(this.state.netDots) !== '') {
      ...
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'ColMon');
  ...
  getCashType() {
    post('statistics/ColMon/queryOptions', { field: 'feeType' }).then(data => {
      if (!data.success) {
        return;
      }
      this.setState({
        cashType: data.module
      });
    })
  }
