...
export class AssetImmovable extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
            ...
            <Button.Group>
              ...
              <Button
                onClick={() => {
                  ...
                  console.log('download all', openUrl);
                  window.open(openUrl);
                }}
              >
  ...
  reOutputArea(pageKey, selectedData) {
    // console.log('output', pageKey, selectedData);
    let selectedArea = [];
  ...
  getInfosByQuering(queryUrl, queryData, cb, delay = 2000) {
    ...
    this.delayAjax = window.setTimeout(() => {
      post(queryUrl, queryData).then((data) => {
        // console.log('data status: ', data, data.success);
        ...
        message.warn('网络错误，请重新提交,error: ' + err);
        this.setState({
          pageLoading: false
        });
        return false;
  ...
  getStateInfo(ifInit, ifPageChange) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'AssetImmovable');
