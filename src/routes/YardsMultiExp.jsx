...
export class YardsMultiExp extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
    ...
    return (
      <section className={publicStyles.section}>
        <QueryHeader>
          ...
          <div hidden={!this.state.ifNetDots} style={{
            wordWrap: 'no-wrap'
          }}>
            <span>网点搜索：</span>
            <Input
              ...
              onChange={(e) => {
                ...
                this.netDotsTimeout = window.setTimeout(() => {
                  {/*console.log('netdots', this.state.netDots)*/ }
                  this.state.netDots !== '' && this.getStateInfo();
                }, 1000);
              }}
            />
          </div>
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      this.getInfosByQuering(postUrl, this.tempPostData, data => {
        ...
        if (ifInit === 'init') {
          ...
          // });
          // console.log('init districtval', this.state.districtVal);
