...
export class Colligate extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
  ...
  render() {
    // console.log('this.state.tablesInfo.module.queryInfoVO.authorityList', this.state.tablesInfo.module &&
    ...
    // console.log('next is new tablesource');
    // console.log('new tablesource', tableSource);
  ...
  getStateInfo(ifInit, ifPageChange) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      this.tempPostData = {
        ...
        "orderBy": this.orderOpt && this.orderOpt.orderby || "",
        "orderRule": this.orderOpt && this.orderOpt.order || ""
      };
      if (ifInit === 'init') {
      ...
      // if (this.state.ifNetDots && this.state.netDots !== '') {
      ...
      //   ]);
      // }
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'Colligate');
