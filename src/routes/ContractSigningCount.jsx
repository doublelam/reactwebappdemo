...
import { ConfigureInfo, QUERY_INFO, CONTRACT_SIGN_SYSTEM } from '../configInfo/ConfigureInfo';
...
export class ContractSigningCount extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
      ...
      <section className={publicStyles.section}>
        <QueryHeader>
          ...
          {/*<div className={publicStyles.rightGap}>
            <span>合同签订系统：</span>
            <Select
              defaultValue={this.contractSignSystem}
              onChange={(val) => {
                this.contractSignSystem = val;
                this.getStateInfo();
              }}
              style={
                {
            ...
            >
              {CONTRACT_SIGN_SYSTEM.map((val, index) => {
                return (
                  <Select.Option
                    key={val}
                  >{val}</Select.Option>
  ...
  getStateInfo(ifInit, ifPageChange) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      let conditionFilter = [];
      if (this.contractSignSystem !== '全部' && this.contractSignSystem !== '') {
        conditionFilter.push({
          "field": "feeType",
          "symbol": "CT",
          "value": this.contractSignSystem,
          "relation": "AND"
        });
      }
      ...
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'RentPact');
