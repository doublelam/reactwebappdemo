...
export class BusinessDetail extends React.Component {
  constructor(props) {
    super(props);
    this.cacheArea = {};

    this.state = {
      dataTime: [moment().add(-1, 'days'), moment().add(-1, 'days')],
      tableData: {
        "message": null,
        "code": null,
        "success": true,
  ...
  render() {
    ...
    const columList = [
      ...
      {
        title: `${this.state.dataTime[0].format('YYYY年MM月DD日')}到${this.state.dataTime[1].format('YYYY年MM月DD日')}数据汇总`,
        dataIndex: 'day',
        key: 'day',
        width: 300
      }
    ...
    return (
      <section className={publicStyles.section} >
        <QueryHeader style={{
          ...
          <div hidden={/all_list|lujing_list/g.test(String(this.state.categorySelect))} className={publicStyles.paddingWith} style={{
            float: 'left'
          }}>
            <span>选择区域：</span>
            <TreeSelect
          ...
          <div style={{
            ...
            <Button style={{ float: 'right' }} onClick={
              (e) => {
                ...
                window.open(`/skynet/statistics/operatequerylist/download?${paramStr}`)
              }
  ...
  getCategorySeries() {
    post('cityView/queryOrganizationalRelationshipByAcl', { "projectType": "skynet_dailylist" })
      .then(data => {
  ...
  getTableData() {
    ...
    this.param = {
      ...
      beginDate: this.state.dataTime[0].format('YYYY-MM-DD'),
      endDate: this.state.dataTime[1].format('YYYY-MM-DD'),
    }
    ...
    this.tableTimeout = setTimeout(() => {
      ...
      post('statistics/operatequerylist', this.param).then(data => {
        if (!data.success) {
          message.warn(data.msg || '网络错误，请重新提交');
          this.setState({
            tableLoading: false
