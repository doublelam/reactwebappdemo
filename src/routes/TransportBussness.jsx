...
export class TransportBussness extends React.Component {
  constructor(props) {
  ...
  render() {
    ...
    tableSource.map((item, index) => {
      ...
      for (let key in item) {
        if (item[key] === null || item[key] === 'null' || item[key] === '') {
          item[key] = '-';
        }
      ...
      <section className={publicStyles.section}>
        <QueryHeader style={{
          ...
          <div className={publicStyles.paddingWith} hidden={!this.state.ifNetDots} style={{
            ...
            >
              <Select.Option value="传化分拨中心">传化分拨中心</Select.Option>
              <Select.Option value="私有分拨中心">私有分拨中心</Select.Option>
              <Select.Option value="网点">网点</Select.Option>
            </Select>
          </div>
        ...
        <Table
          ...
          scroll={{ x: columns.length * 130, y: 'calc(100vh - 370px)' }}
          size="small"
          bordered
          onChange={(pagination, filters, sorter) => {
            {/*console.log('get sorter', sorter);*/ }
            this.orderOpt = {
  ...
  getStateInfo(ifInit, ifPageChange) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      let conditionFilter = [];
      if (this.state.ifNetDots && ObjOperate.trimExtra(this.state.netDots) !== '') {
        conditionFilter.push({
          "field": "warehouseName",
          "symbol": "CT",
          "value": ObjOperate.trimExtra(this.state.netDots) || '',
          "relation": "AND"
        });
      }
      ...
      if (this.state.ifNetDots && this.state.netType !== '' && this.state.netType !== undefined) {
        conditionFilter.push({
          "field": "warehouseType",
          "symbol": "CT",
          "value": this.state.netType || '',
      ...
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'TransportStatsSys');
