...
import { ConfigureInfo, QUERY_INFO, SELECT_TYPE, PAYMENT_TYPE } from '../configInfo/ConfigureInfo';
...
export class IncomeCount extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dateType: 'month',
      ...
      <section className={publicStyles.section}>
        <QueryHeader>
          ...
          <div className={publicStyles.paddingWith} style={{
            ...
            >
              {this.state.selectTypes.map((val, index) => {
                return (
                  <Select.Option
                    key={val.type || val.partyName}
                  >{val.type || val.partyName}</Select.Option>
                );
              })}
            </Select>
          </div>
          <div className={publicStyles.paddingWith} style={{
            ...
            >
              {this.state.paymentTypes.map((val, index) => {
                return (
                  <Select.Option
                    key={val.type || val.partyName}
                  >{val.type || val.partyName}</Select.Option>
  ...
  getSelectTypes() {
    [{ stateName: 'selectTypes', url: 'JobPassFinance_Type' },
    { stateName: 'paymentTypes', url: 'JobPassFinance_smallType' }].map(v => {
      post('statistics/welcome/queryExtOptions', { field: v.url })
        .then(data => {
          if (!data.success) {
          ...
          let stateObj = {};
          stateObj[v.stateName] = data.module
  ...
  getStateInfo(ifInit) {
    ...
    this.getStateInfoTimeout = setTimeout(() => {
      ...
      let postUrl = ConfigureInfo.getQueryUrl(this.state.portType, this.state.dateType, 'Finance');
