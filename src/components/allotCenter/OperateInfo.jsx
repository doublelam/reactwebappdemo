...
class OperateInfo_ extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      tableSource: [],
    ...
    this.useId = 'allotCenterOperationId';
  }
  ...
  render() {
    ...
    return (
      <section>
        ...
        <TableList
          ref="table"
          url="allotCenterOperation/getAllotCenterOperations"
          excelOpt={{
            excelId: this.useId,
            excelPartUrl: '/tmsBossAdmin/allotCenterOperation/getAllotCenterOperationExcel',
            excelAllUrl: '/tmsBossAdmin/allotCenterOperation/getAllotCenterOperationExcel'
          }}
          resetColumn={(columns) => {
  ...
  handleEdit() {
    this.props.form.validateFieldsAndScroll((err, val) => {
      ...
      let params = val;
      const URL_MAP = {
        add: 'allotCenterOperation/addAllotCenterOperation',
        edit: 'allotCenterOperation/modifyAllotCenterOperation'
      };
  ...
  deleteOperate(id) {
    post('allotCenterOperation/deleteAllotCenterOperation', { id: id }).then(data => {
      if (!data.success) {
        if (data.code === '9999') {
          message.warn('暂无权限！');
          return;
...
export const OperateInfo = Form.create()(OperateInfo_);
